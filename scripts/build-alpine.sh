...
# Only sed SUBDIRS that CONSUME liblzma, not liblzma's own Makefile
# (where liblzma.la is the target being built).
( cd "$BUILD_DIR" && \
	find src/xz src/xzdec src/lzmainfo src/scripts -name Makefile -print0 | xargs -0 sed -i \
